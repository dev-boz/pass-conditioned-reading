...
| 5 | **Home-building scaffold** (was: typed slots; reframed by P1b) | RECORD (gives REPLACE its matching target) | **P1b RUN 2026-07-28:** with a matching home, adder lands 8–9/10 and relation 4/10 through the strict guard; **slot syntax is NOT load-bearing** — a name-mention entry receives values as well as a `[TBD:]` slot (ctrl8 ≈ slots; size-matched padding 0–1/10; zero fabrication in 30 draws). Kills the gauntlet's slot-grammar/quote-matching worries by making them unnecessary. Training target: Phase-A scaffold builds value-shaped, module-keyed homes for later tiles WITHOUT oracle help. **Maintainer 07-28, two constraints:** (a) *k-locator homes* — future form annotates the home with where the fill will arrive ("find at roughly pass k"), fusing the 0.03% locator-hint gap with home-building and giving the position channel its first real job (see 15/F2); (b) *generalization guard* — the home-building rubric must name a CLASS ("entities you expect details about later"), never this fixture's shape (modules/.py), and any gate must eval on ≥2 document kinds (code + prose corpus exists) so we train the strategy, not this scenario | measure on archives: how often did the student's own scaffold create a home the later tile could land in? then the confirming n≥10 run (three-tier rule) | **LIVE — the lead training lever** |
