...
| **queryfirst** (question moved *before* the state at close) | **0/10** | prefill order — does query-conditioned retrieval at close help? |
| **padded** (operand lines kept at original positions; every other line → digit-free neutral filler, length preserved) | **0/10** | length vs distraction, disentangled |

The **padded** probe (2026-07-21) dissociates the two close-side stories the `distilled` probe conflated. Compose stays 0/10 at full length even with distractors gone — but the *wrong answers move*: seed 2 goes 86 (decoy value) → **988** (the right table's value), seed 5 → 193 (right table, wrong row), seeds 7/42 → 1365 (the one operand their state held). All no-operand states stay `None` (clean negative control). Reading: **distractor content governs where retrieval lands; length governs whether two-operand composition fires at all** — the full chain fired only in the 2-line distilled state. Both joints must be fixed at 1.5B (or scale must rise). This confirms the maintainer's small-model long-context-degradation hypothesis for the compose step specifically, and is logged as levers 2/4/9 in `STAGEA-EXPLORATION-LEDGER.md`.

The **queryfirst** probe (maintainer hypothesis, 2026-07-21) puts the scoring question *before* the 7.7K-token state so the model reads the brief already knowing what it seeks. It did **not** help (0/10; seed 2 went 86→no-answer, seed 5 stayed 23). Combined with `retrieve` (also 0/10), this is a stronger result than distillation alone: neither a prompt-level retrieval instruction *nor* prefill reordering rescues composition — **only physically removing the distractors does**. The retrieval failure is structural in the flat-text state, not addressable by how the closing turn is phrased or ordered. Scope: this tests prefill order at the **compose/close** step on frozen weights only; the reading-pass order (a training-time choice) is a separate, untested, and legitimate lever that would require retraining to probe.
