...
> **Status addendum, 2026-08-06.** This summary is the record through P7 (June 2026) and stands unedited below. Since then: the first trained student ran and its pre-registered gate failed (0/10) - and the diagnosis that followed showed the scored demand was never trained and the implemented schedule was never the architecture's pyramid, so that verdict binds the instantiation, not the design. The pyramid was then built, an edit guard was added after measuring the edit pathology, and the pipeline completed end to end for the first time - replicated on held-out seeds under a pre-registered confirm: a 1.5B maintains a bounded state over a 65K-token document that an untrained 3B answers a cross-module question from, and every larger reader up to a frontier model answers on exactly the same draws (the state is exactly sufficient; the open headroom is all in the recorder). Scope: one saturated fixture, no budget-matched baseline yet, and pass-conditioning itself is still unproven - the position channel measured inert twice, on a student never trained on a resolution ladder. Current record: the repo README's status update and `experiments/e2prime/PYRAMID-COMPRESSOR-RESULTS.md`. One framing correction that post-dates everything below: DiSCo targets any model at any window size - the use case is sessions of multiple millions of tokens that no context window contains - and the small local models in these experiments are the compute-bound test rig, not the point. And a process note, same date: the hands-on work in this program - code, runs, analysis, drafts - is AI-executed under my direction and review; the README's "How this work is actually done" section says it plainly.
