...
2. **Regression replay (completed 2026-07-10, D31 amendment):** the archived retention-failure instances were replayed against the new harness. Finding: the archive holds two distinct shapes — **slot-repurposing** (seed-2: the surcharge entry overwritten by an unrelated module description) and **content-omission inside a legitimate same-entry rewrite** (seed-42). The slot-repurposing instance is **blocked under guard=`strict`** (the table survives); the id-misaddressing shape from the documentation (`REPLACE S3: FUNCTIONS=…`) is blocked under `block`/`strict` (unit fixture). Content-omission is **out of P-A scope by design**: no addressing guard can catch it without the harness knowing which content matters (the forcing line); it is assigned to §5(d) and measured, not assumed fixed, in Stage A.
3. **No false-block regression (completed 2026-07-10):** all replayable archived known-good transcripts (P1, three arms, 157 destructive ops) show **0 blocked / 0 redirected under `strict`** after the duplicate-text batch exemption. P2/P3 lack per-pass snapshots and are not replayable; the evidence base is P1 + unit fixtures.
4. **E2′ guard mode = `strict`** (quoted ops redirect-by-content; quote-less near-zero-overlap rewrites refused). Thresholds and the descriptor-stopword list freeze at data generation.

P-A is satisfied as of the D31/D32 implementation. If later evidence reopens it, fix the harness — harness iteration is unlimited (architecture-side, always permitted under the forcing line).
...
| **≤ 1/10** | **ARCHITECTURE GATE.** The text-state edit-op instantiation is gated — given P-A (clobber fixed), P-B (fork addressed), P-C (training healthy), and a legitimate need-class framing, training did not move the chain. | No further training spend on this instantiation. **The pre-named third option (maintainer, 2026-07-10): the native-architecture design (docs/DISCO-NATIVE-ARCHITECTURE.md — latent slot-bank integration state, AdaLN pass-conditioning), which holds all relevant information internally before output and therefore eliminates the multi-edit sequence entirely. It answers the one question a dual failure leaves open: "is the multi-edit sequence the problem?"** Alternatively program stop. §5's component attribution names the blocked joint; it cannot soften the gate. |
...
**Standing test-validity caveat (maintainer, 2026-07-10).** The split-unit chain is a *concession test*: it was adopted because it has a concrete, killable outcome, not because it directly measures the architecture's stated goal (maintaining the nuance of long sessions). A verdict on this gate is therefore a verdict on the instantiation's mechanics, not on the goal. The better-aligned test exists as a draft (docs/NUANCE-EXPERIMENT-SHAPE.md — the non-local nuance experiment, M-NL: score the decision the nuance forces, not the nuance) and is **deferred until after E2′** by the maintainer's call. If this gate lands fail-or-gray, the pre-registered follow-on is to build that test and run it as a trained-vs-untrained comparison — the Stage-A student and its floor are exactly the pair M-NL needs.
